...
> **Design rule: Inbound brokerage data uses `extra="allow"` and no value-range constraints.** The brokerage is the source of truth. All inbound Pydantic models accept and preserve unknown fields on `model_extra` — we never reject or discard data the brokerage sends. If the API adds a field we haven't modeled, it flows through the system intact. Inbound models must NOT use Pydantic value constraints (`ge`, `le`, `gt`, `lt`) — these reject valid brokerage data (e.g. negative futures marks, edge-case Greeks). Value validation belongs in analytics/display layers, not ingestion. `extra="forbid"` is reserved exclusively for **outbound** messages we construct (DXLink protocol, streamer connect/heartbeat), where it catches our own typos before they hit the wire. Any exception to this rule requires a clearly documented design objective.
...
- **Unbounded intake — a known, monitored weak point (TT-161)** — The per-channel market-data queues have no `maxsize`. Nothing is ever dropped; instead, any sustained interval where events arrive faster than the consumer drains becomes growing delay, and because delivery stays in order the lag only clears when the producer goes quiet. This happened twice: TT-157 (per-event InfluxDB HTTP writes starved the consumer; pipeline hours behind for most of the TT-156 program) and a 2026-08-28 recurrence (~100x forming-candle re-writes; 3.5 h behind from 10:30). Both had a concrete consumer-cost cause, both were removed (TT-157 batched writer, TT-159 sealed-bar candle writes), and the pipeline has since run with no accumulation into the close. The queue itself was deliberately left unbounded rather than fenced with drop or conflation logic: the Redis-for-live / InfluxDB-for-history split is the architectural answer to volume, and whether backlog ever recurs is a question for TT-158 pipeline-health observability (queue depth, publish lag) to settle with evidence. Keep-newest conflation and bounded-queue drops are deferred until that monitoring shows a queue-depth trend across full sessions.

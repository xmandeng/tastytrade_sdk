...
main() {
    ...
    # Wait for the transcript to contain an assistant message.
    # The Stop hook fires ~45ms after the assistant response, which can
    # race with the file write buffer. Poll the last line until it has
    # role "assistant", or bail after a timeout.
    local wait_attempt=0
    local max_wait=10  # 10 x 50ms = 500ms max
    while [ "$wait_attempt" -lt "$max_wait" ]; do
        local last_role
        last_role=$(tail -1 "$transcript_path" | jq -r '
            if type == "object" and has("message") then .message.role
            elif type == "object" then .role
            else "unknown" end' 2>/dev/null)
        if [ "$last_role" = "assistant" ]; then
            debug "Transcript ready after $((wait_attempt * 50))ms (last_role=assistant)"
            break
        fi
        sleep 0.05
        wait_attempt=$((wait_attempt + 1))
    done
    if [ "$wait_attempt" -ge "$max_wait" ]; then
        log "WARN" "Transcript wait timed out after 500ms (last_role=$last_role)"
    fi
    ...
    while IFS= read -r line; do
        ...
        if [ "$role" = "user" ]; then
            if [ "$(is_tool_result "$line")" = "true" ]; then
                ...
                if [ "$(echo "$current_assistant_parts" | jq 'length')" -gt 0 ]; then
    ...
    # Check parts length only; msg_id may be empty when transcript lacks .message wrapper
    if [ "$(echo "$current_assistant_parts" | jq 'length')" -gt 0 ]; then
        local merged
        merged=$(merge_assistant_parts "$current_assistant_parts")
        write_tmp "$merged"
